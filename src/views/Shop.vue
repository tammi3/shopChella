...
<template>
  <div class="flex flex-col gap-3 py-16">
    ...
    <div class="px-10 grid items-end grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
      <div v-show="!loading" v-for="product in filteredProducts" class="mx-auto bg-white overflow-hidden">
        <div>
          <div class="w-full flex justify-center overflow-hidden">
            <img class="w-full h-full object-fit" :src="product.image" :alt="product.name + ' image'" />
          </div>
          <routerLink :to="'/Product/' + product.id" class="p-2 flex flex-col justify-center items-center space-y-1">
      ...
      <div v-for="skeleton in filteredProducts" v-if="loading"
