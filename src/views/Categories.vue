...
<template>
  <div class="pl-410 pr-10 py-20 flex flex-col gap-10">
    ...
    <div class="w-full justify-center grid grid-cols-4 gap-10">
      <div
        class="flex flex-col gap-4 justify-end items-center border border-gray-400 border-solid shadow-md"
        v-for="product in filteredProducts"
      >
        <router-link class="flex items-center" :to="'/Product/' + product.id"
        ...
        <div class="p-4 flex flex-col gap-3 w-full border-t border-gray-400">
          <router-link
            ...
            class="font-semibold text-md uppercase font-Ubuntu"
            >{{ product.title }}</router-link
          >
          <div class="flex justify-end w-full">
            <span
              class="text-gray-700 w-32 h-10 bg-purple rounded-lg text-sm flex justify-center items-center"
              >${{ product.price }}</span
            >
          </div>
        </div>
